		...
		if (RAIN) {
			for (int x = 0; x < points.length; ++x) {
				for (int y = 0; y < points[x].length; ++y) {
					...
					if (y >= 1) points[x][y].addNeighbor(points[x][y - 1]);
				}
			}
		} else {
			for (int x = 0; x < points.length; ++x) {
				for (int y = 0; y < points[x].length; ++y) {
					...
					LinkedList<Vector2D> neighbours = pointVector.getNeighbours(length, height);
					for (Vector2D n : neighbours) {
						points[x][y].addNeighbor(points[n.x][n.y]);
					}
				}
			}
		}
				...
				if (points[x][y].getState() != 0) {
					...
					case 1:
						if (RAIN)
							...
							g.setColor(new Color(0x8e2211));
						break;
					case 2:
						g.setColor(new Color(0xbbbbff));
						break;
					case 3:
